...
export async function GET(
  ...
  try {
    ...
    const statusRes = await fetch(`https://api.valsea.ai/v1/voice-security/${jobId}`, { headers: authHeader });
    ...
    const resultRes = await fetch(`https://api.valsea.ai/v1/voice-security/${jobId}/result`, { headers: authHeader });
    const resultData = await resultRes.json();
    if (resultRes.ok) cacheResult(jobId, resultData);
    return NextResponse.json(resultData, { status: resultRes.status });
  } catch (error) {
    return NextResponse.json(
      { error: error instanceof Error ? error.message : 'Voice security poll failed' },
      { status: 500 },
    );
  }
}
